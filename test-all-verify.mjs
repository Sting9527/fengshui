...
const jishenList = (JISHEN_MAP[dayGan] || []).slice(0, 6).join('、');
const xiongshenList = (XIONGSHEN_MAP[dayGan] || []).slice(0, 4).join('、');
...
SHICHEN_NAMES.forEach((name, index) => {
    const isJi = index % 2 === 0;
    console.log(`${name}时: ${isJi ? '凶' : '吉'}`);
});
